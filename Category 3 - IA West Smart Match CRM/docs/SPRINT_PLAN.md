...
|------|-------|-------------|
| A0.1 Environment setup | 1.5 | Python 3.10+ venv, install deps (streamlit, openai, pandas, plotly, beautifulsoup4, playwright), configure `.env` with `OPENAI_API_KEY`, scaffold project structure under `src/` |
...
| A0.3 Embedding pipeline — speakers | 2.5 | Generate OpenAI text-embedding-3-small vectors for all 18 speaker profiles. Compose embedding text from: `f"{expertise_tags} {title} {company} {board_role}"`. Cache embeddings as `.npy` or `.pkl` to avoid re-calling API. |
| A0.4 Embedding pipeline — events + courses | 2.5 | Generate embeddings for 15 CPP events and 35 course sections. Compose event embedding text from: `f"{event_name} {category} {volunteer_roles} {primary_audience}"`. For courses: `f"{title} {guest_lecture_fit}"`. Cache results. |
...
- [ ] 77 embedding vectors generated and cached locally
...
| Task | Hours | Description |
|------|-------|-------------|
| A2.1 Web scraping pipeline — core | 5.0 | Build modular scraper: `scrape_university(url, method="bs4"|"playwright") -> raw_html`. Implement for all 5 universities. Add JSON file cache: `{url: {html: ..., scraped_at: ..., ttl: 24h}}`. Respect robots.txt. Rate limit: 1 request per 5 seconds. |
| A2.2 LLM extraction pipeline | 4.0 | GPT-4o-mini prompt: given raw HTML/text, extract structured JSON array of events: `{event_name, category, date_or_recurrence, volunteer_roles, primary_audience, contact_name, contact_email, url}`. Few-shot examples in prompt. Parse and validate JSON output. Handle extraction failures gracefully (return empty + log). |
| A2.3 Discovery tab — UI | 3.5 | University selector (5 pre-loaded + "Custom URL" input). "Discover Events" button triggers scrape + extract. Results table showing extracted events with "Add to Matching" button. Status indicators: "Cached" vs. "Live Scrape" vs. "Failed — Using Cached". Show scrape timestamp. |
| A2.4 Outreach email generation | 3.0 | GPT-4o-mini prompt: given speaker profile + event details + match score breakdown, generate personalized outreach email. Include: subject line, greeting referencing speaker's specific expertise, event details, value proposition for volunteering, call-to-action. Add "Copy to Clipboard" button. Email preview panel in Matches tab. |
...
| A4.3 Edge case hardening — Day 12 | 3.0 | Test: What happens when scraping fails mid-demo? (fallback to cache). What happens when OpenAI API times out? (show cached explanation). What happens when user adjusts all weights to 0? (show error message). What happens with custom URL that returns garbage HTML? (graceful error). |
...
| A4.5 Streamlit Cloud deployment — Day 12 | 2.0 | Deploy to Streamlit Community Cloud. Configure secrets (OPENAI_API_KEY). Test on cloud: verify all features work within 1GB RAM limit. Note: Playwright may not work on cloud — use pre-cached scrape results as default, live scrape only when running locally. |
...
| A4.9 Day-of prep — Day 14 | 3.0 | Pre-warm all caches: run every scrape, generate every explanation, generate every email. Verify demo mode works. Check internet connectivity backup plan. Charge laptop. Test projector output. |
...
| OpenAI API down | API timeout > 10s | All explanations, emails, and extraction results pre-cached. Switch to demo mode. |
